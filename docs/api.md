...
| Method | Path | 설명 |
|--------|------|------|
| `GET` | `/api/v1/documents` | page/pageSize/q/category로 검색 가능한 문서 목록 (`fileKey`, `fileUrl` 포함) |
| `POST` | `/api/v1/documents` | JSON 본문으로 단일 문서 생성 |
| `POST` | `/api/v1/documents/bulk-ingest` | 문서 배열을 한 번에 업로드 |
| `GET` | `/api/v1/documents/{id}` | 단일 문서 조회 |
| `PUT` | `/api/v1/documents/{id}` | 단일 문서 수정 |
| `DELETE` | `/api/v1/documents/{id}` | 단일 문서 삭제 |
| `POST` | `/api/v1/documents/reindex` | `{documentIds:[...]}`로 Qdrant 재색인 |
| `GET` | `/api/v1/documents/stats` | 전체 문서 통계 |
| `POST` | `/api/v1/documents/upload` | `multipart/form-data`로 파일 업로드 → S3 저장 + 텍스트 추출 |
